...
input_tensor_shapes = tf.placeholder(tf.float32, shape=[1, imageSize, imageSize, 3])
...
'''coreml_model = tfcoreml.convert(
        ...
        image_input_names=[tf.placeholder(tf.float32, shape=[1, imageSize, imageSize, 3])],
...
print(img)
image = tf.placeholder(tf.float32, shape=[1, imageSize, imageSize, 3], name = 'img');
print(image)
...
graph_def_file = "./models/frozen_model.pb"
input_arrays = ["image"] #["input_tensor_shapes"]
output_arrays = ["heatmap"] #["MobilenetV1/Predictions/Softmax"]
...
tflite_convert \
  --graph_def_file=models/frozen_model.pb \
  --output_file=models/script_model.lite \
